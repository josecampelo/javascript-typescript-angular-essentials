// Desafio: Preparação de Time Ranqueado com async/await
...
async function preparaJogador(jogador) {
    console.log(`Preparando o jogador ${jogador.nome}`)
...
async function prepararEquipe(jogadores) {
    for(let i = 0; i < jogadores.length; i++) {
        await preparaJogador(jogadores[i])
    }
}
